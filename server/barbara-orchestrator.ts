...
const TOPIC_OVERLAP_TIMEOUT_MS = 10000;
...
export async function analyzeTopicOverlap(
  ...
  // If no previous summaries, no overlap possible
  if (!previousSummaries || previousSummaries.length === 0) {
    return { hasOverlap: false, overlapSummary: null, relevantQuestionNumbers: [] };
  }

  // Filter out empty summaries
  const validSummaries = previousSummaries.filter(
    s => s && s.respondentSummary && s.respondentSummary !== "Minimal or no response provided."
  );
  ...
  const summariesContext = validSummaries
    .map(s => `Q${s.questionIndex + 1}: "${s.questionText}"
  Response: ${s.respondentSummary}
  Key Insights: ${s.keyInsights.join("; ")}`)
    .join("\n\n");

  const systemPrompt = `You are Barbara, an interview analysis assistant. Determine if an upcoming interview question's topic has already been touched on by the respondent in previous answers.

You must respond with a JSON object:
{
  "hasOverlap": true/false,
  "overlapSummary": "Brief 1-2 sentence description of what the respondent already mentioned that relates to this question" or null if no overlap,
  "relevantQuestionNumbers": [array of question numbers (1-indexed) where overlap was found] or []
}

Only mark hasOverlap as true if the respondent SUBSTANTIVELY addressed part of the upcoming question's topic. Minor tangential mentions don't count.
Keep overlapSummary concise and actionable for the interviewer.`;

  const userPrompt = `UPCOMING QUESTION TO ASK:
"${upcomingQuestionText}"

PREVIOUS ANSWERS FROM RESPONDENT:
${summariesContext}

Has the respondent already touched on the topic of the upcoming question? If so, what did they say?`;

  try {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error("Topic overlap analysis timeout")), TOPIC_OVERLAP_TIMEOUT_MS);
    });
    ...
    return {
      hasOverlap: Boolean(parsed.hasOverlap),
      overlapSummary: parsed.overlapSummary || null,
      relevantQuestionNumbers: Array.isArray(parsed.relevantQuestionNumbers) ? parsed.relevantQuestionNumbers : [],
    };
  } catch (error) {
