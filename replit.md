...
-   **Audio Playback Pipeline**: `use-audio-playback.ts` uses absolute-time scheduling to eliminate clicking/crackling at chunk boundaries. Key architecture: `drainQueue()` pre-schedules chunks up to 200ms ahead using `source.start(absoluteTime)` with full chunk duration (no overlap/fade — OpenAI's PCM stream is sample-continuous at chunk boundaries, so fades are unnecessary and would compress speech tempo). Each chunk gets a per-source `GainNode` at unity for clean barge-in fade-out via `cancelAndHoldAtTime` (with Safari fallback). `isDrainingRef` boolean prevents concurrent drain races; `drainGenerationRef` is only incremented by `stopAiPlayback`/teardown for explicit invalidation. Generation tokens (`playbackGenerationRef`) prevent stale `onended` callbacks from corrupting state. Pure scheduling utilities in `client/src/lib/audio-scheduling.ts` with tests in `client/src/lib/__tests__/audio-scheduling.test.ts`.
...
-   **AI Persona Generation**: A two-phase pipeline researches target populations via LLMs and synthesizes diverse personas, with post-generation diversity validation. Population briefs are persisted to the database with provenance tracking (`isUngrounded` flag, `populationBriefId` FK on personas). Users can retrieve and reuse existing briefs via a brief selection UI in the generation dialog, skipping the 10-minute research step. The brief selection view shows expandable cards with demographics, suggested profiles, behavioral patterns, biases, domain knowledge, and sources. Research uses streaming (`openai.responses.stream()`) with a 300s inactivity guard to avoid Node.js undici HTTP timeouts. On server restart, orphaned `researching`/`synthesizing` tasks are marked `"interrupted"` by `cleanupOrphanedResearchAndSynthesis()` in `server/storage/simulation.ts`. The client detects interrupted status during polling and shows a Retry/Cancel UI, with restart endpoints (`POST .../restart`) that reset status and re-fire the background task.
