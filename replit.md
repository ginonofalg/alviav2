...
-   **Branded Interview Welcome Page**: Projects have an optional `brandingLogo` (base64 data URL, text column). Users upload a logo via the "Branding" tab on the project edit page, which opens a crop dialog (react-easy-crop, 1:1 aspect, 256x256 WebP output). On the interview welcome page, if a logo exists, it renders prominently with a "Powered by Alvia" badge below; otherwise a default Bot icon is shown. Consent and interview-ready pages use a Bot icon instead of the old Mic icon. The `GET /api/interview/:sessionId` and resume endpoints include `brandingLogo` from the project. Key files: `client/src/components/ImageCropDialog.tsx`, `client/src/components/BrandedWelcomeAvatar.tsx`, `client/src/lib/image-utils.ts`.
